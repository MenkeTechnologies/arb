...
use crate::query::{self, QueryOp, QueryResult};
use crate::spec::{self, Spec};
use crate::stream::StreamState;
use crate::{cache, parser, tui};
...
fn run_filter(pat: &str, exact: bool, no_sort: bool, look: &crate::fzf::Look) -> io::Result<()> {
    use std::io::Read;
...
/// Run one `arb` invocation inside a host process and return its exit status.
///
/// This is the entry point for a host that has no fork to spend — the `arb`
/// shell builtin in zshrs-native, where arb is linked into the shell and its
...
pub fn run_argv(argv: &[String]) -> i32 {
    crate::hosted::run(|| match run(argv) {
        Ok(()) => 0,
        // A closed downstream pipe is how `arb … | head` ends, not a failure.
...
fn run(full_argv: &[String]) -> io::Result<()> {
    ...
    let raw: Vec<String> = full_argv.iter().skip(1).cloned().collect();
    if let Some(code) = crate::pkg::dispatch(&raw) {
    ...
    let mut argv: Vec<String> = full_argv.to_vec();
    if argv.iter().any(|a| a == "--fzf") {
        let env = crate::fzf::env_args();
        argv.splice(1..1, env);
    }
    ...
    // options.
    let fzf_look = crate::fzf::Look::parse(&argv);
    ...
    if cli.list_themes {
        ...
        let mut out = io::stdout().lock();
        for (name, pal) in crate::theme::THEMES {
    ...
    if let Some(name) = &cli.set_theme {
        match crate::theme::set_config_default(name) {
            Ok(()) => {
    ...
    match cli.theme.as_deref() {
        ...
        Some(name) => match crate::theme::by_name(name) {
        ...
        None if !spec.theme_off && spec.theme.is_none() => {
            spec.theme =
                Some(crate::theme::config_default().unwrap_or_else(crate::theme::default_palette));
        }
        None => {}
    }
    ...
    if cli.html {
        print!("{}", crate::web::render_html(&spec));
        return Ok(());
    }

    if cli.test {
        // Run the in-language `test { … }` blocks headlessly (TAP), exit 0/1.
        let report = crate::testrun::run(&spec.tests);
        print!("{}", report.text);
        crate::hosted::exit(if report.failed == 0 { 0 } else { 1 });
    }
    ...
    if tui::events_available() && !downstream_reshape {
        ...
        let err_pane = if spec.spawn_pty && run_pipeline.is_none() && producer.is_some() {
            ...
            match crate::pty::spawn_pty_producer(prod, state.clone()) {
                Ok(writer) => {
        ...
        };
        {
            ...
            if !spec.actor_refs.is_empty() {
                match crate::actor::Session::build(&spec.actors, &spec.actor_refs) {
            ...
            c.theme = spec.theme;
            c.theme_idx = spec.theme.map(crate::theme::index_of).unwrap_or(0);
...
fn dump_tokens(cli: &Cli) -> Result<(), String> {
    ...
    for (tok, off) in crate::lexer::lex(&src, false).map_err(|e| e.to_string())? {
        println!("{off}\t{tok:?}");
    }
    Ok(())
}
...
fn dump_ast(cli: &Cli) -> Result<(), String> {
    let src = dump_src(cli)?;
    let cmds = crate::parser::parse(&src).map_err(|e| e.to_string())?;
    println!("{cmds:#?}");
    Ok(())
}
...
mod tests {
    ...
    fn presentation_flags_are_parsed_then_kept_from_clap() {
        ...
        let look = crate::fzf::Look::parse(&argv.iter().map(|s| s.to_string()).collect::<Vec<_>>());
        assert_eq!(look.layout, crate::fzf::Layout::Reverse);
