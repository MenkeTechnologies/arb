...
const JQ_CALLS: &[&str] = &[
    ...
    "modulemeta",
    "toboolean",
    // The yq surface's CALL forms. `;` is jq's argument separator and arb's
    // command terminator, so `with(.a; .b = 5)` was cut in half and reported as
    // `unknown verb \`with(.a\`` — a name arb HAS, refused by the lexer before
    // the engine ever saw it. None of these is an arb verb.
    "with",
    "ref",
    "ireduce",
    "with_dtf",
...
const JQ_BARE: &[&str] = &[
    ...
    "recurse",
    "toarray",
    // The yq surface's BARE forms, for the same reason the call forms are listed
    // above: a MULTI-token command such as `.a | line_comment` must reach the
    // engine as one atom. None is an arb verb.
    "anchor",
    "alias",
    "tag",
    "style",
