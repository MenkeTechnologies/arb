//! Running arb inside somebody else's process.
//!
//! The `arb` binary owns its process: it can `exit()` from anywhere in the
//! command line layer, leave the terminal in whatever mode the TUI put it, and
//! let the kernel reclaim the rest. A host that dispatches `arb` as one command
...
//! should grow a dependency on the other to be embeddable.
