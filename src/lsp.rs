...
const CORPUS: &[(&str, &str, &str, &str)] = &[
    ...
    ("logview", "Widget", "Log tail tinted by detected level (ERROR/WARN/INFO/DEBUG); theme-aware.", "logview .log  ·  source .log { in }"),
    ("heatmap", "Widget", "Numeric series as a grid of shade-scaled cells (GitHub-contribution style).", "heatmap .h  ·  source .h { in; field lat }"),
    ("treemap", "Widget", "Tally/count pairs as proportional labeled rectangles across the palette.", "treemap .t  ·  source .t { in; field svc; tally }"),
    ("gantt", "Widget", "`label start end` lines as horizontal bars on a shared time axis.", "gantt .g  ·  source .g { in }"),
    ("diff", "Widget", "Lines tinted by leading char: + add (green), - remove (red), @ hunk.", "diff .d  ·  source .d { in }"),
    ("logo", "Widget", "The ratatui braille wordmark, centered — a decorative splash.", "logo .brand"),
    ("clear", "Widget", "A blank spacer (occupies a grid cell without drawing) to open space.", "clear .gap"),
    ("rule", "Widget", "A divider line in the theme accent (horizontal, or -dir vertical).", "rule .sep"),
