...
out.yaml out.json out.props out.xml out.csv out.tsv [INDENT]
                         re-serialize the stream (yq's -o= / -I) ✅
...
**With that, the jq leg reports ZERO divergences.** So does every other leg: the
whole run is 825 probes, 825 pass, 0 diverged, no allowlist. The script exits
with the divergence count, so a clean run is an exit status and not a summary
line anyone has to read.
